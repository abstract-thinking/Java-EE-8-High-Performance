package com.example.monitoring.impl;

// import com.github.rmannibucau.cdi.monitoring.api.Monitored;

import com.example.monitoring.api.Monitored;
...
public class PerformanceExtension implements Extension {
    ...
    void loadConfiguration(@Observes final BeforeBeanDiscovery beforeBeanDiscovery) {
        try (final InputStream configStream =
                     Thread.currentThread().getContextClassLoader().getResourceAsStream("performances.properties")) {
        ...
        isEnabled = Boolean.parseBoolean(configuration.getProperty("enabled", "true"));
    }
